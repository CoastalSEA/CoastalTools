...
%     
    ...
    properties (Transient)
        %if this list changes case numbers in runModel need to be updated
        %along with uses of id_model for model selection (eg after dstable assignment)
        MenuList = {'Volumes','Shore volumes','Shore position','Shore lines',...
                    'Profile variance','BVI profile set','Crenulate Bay',...
                    'Shore profile','Dean profile'}                     
        ModelName = {'Volumes','ShoreVolumes','ShorePosition','ShoreLines',...
                     'ProfileVariance','BVI'}
    end
    ...
    methods (Static)                 
        function runModel(mobj,src)
            ...
            switch id_model
                ...
                case 5  %variance of shoreline at range of profile elevations
                    output = profileVariance(obj,mobj);
                    % if ~isempty(output)
                    %     src = findobj(mobj.mUI.Tabs.Children,'Tag','Profiles'); 
                    %     headtxt = sprintf('Profile: %s\nFor: %s',...
                    %                   output.source,output.metatxt);
                    %     obj.TabOutput.table = output.table;
                    %     obj.TabOutput.headtxt = headtxt;          
                    %     tablefigure(src,headtxt,output.table);
                    % end                  
                case 6 %beach vulnerability index
                ...
                case 7 %crenulate bay plots
                    crenulateBay(obj,mobj);
                    output = [];
                case 8  %plot an idealised profile and closure depths
                    src = findobj(mobj.mUI.Tabs.Children,'Tag','Profile');                    
                    bmvProfile(obj,mobj,src);
                    output = [];
                case 9  %plot the Dean profile
                    src = findobj(mobj.mUI.Tabs.Children,'Tag','Profile'); 
...
%%
        function output = volumes2matrix(obj,mobj)
            ...
            %plot source data as a space-time profile plot
            plotSpaceTime(obj,time,profnames,datavar,vardesc);
...
%%
        function output = profileVariance(obj,mobj)
            %set up call to ct_beachvariance and handle output
            output = [];
            %prompt user to select profiles to be used (multiple profiles)
            caserec = getMinShorelineSet(obj,mobj,2);  %2=min number of profiles to select 
            if isempty(caserec), return; end
            npro = length(caserec);
            [results,meta] = ct_beachvariance(obj,mobj,caserec);
            if isempty(results), return; end

            %setup output for writing to stored dataset
            output.results = results;
            output.modeltime = meta.time;
            output.zdata = meta.zdata; 
            mnx = minmax(meta.zdata);
            output.metatxt = sprintf('zLevels between %.3f and %.3f',mnx(1),mnx(2));
            output.source = sprintf('using %d profiles at %.1f levels',...
                                                  npro,numel(meta.zdata));
            %list of profiles used to construct shoreline
            output.proflist = meta.proflist;

           plotProfileVariance(obj,results,meta);
           if ~meta.issave, output = []; end %results do not need to be saved
        end

%%
        function output = getBVindex(obj,mobj)
            %set up call to ct_beachvulnerability and handle output
            %setup output for writing to stored dataset
            output = [];
            %prompt user to select profiles to be used (multiple profiles)
            caserec = getMinShorelineSet(obj,mobj,2);  %2=min number of profiles to select 
...
%%
        function plotProfileVariance(~,results,meta)
            %contour plot of beach movement for multiple profiles and elevations
            z = meta.zdata; 
            
            np = numel(meta.proflist);
            p = 1:np;
            res = results{3};         %shoreline position
            resmean = mean(res,1,'omitnan');
            anom = res-resmean;
            [sd,~] = std(anom,0,1,'omitnan');
            mx = max(anom,[],1,'omitnan');

            newres = {squeeze(mx),squeeze(sd)};
            ...
            t = tiledlayout(hf,2,1,'TileSpacing','compact','Padding','compact');

            ptxt = {'Maximum position anomaly (m)','Std.Dev. position anomaly (m)'};
            for i=1:2
                setPlot(i,p,z,newres{i},ptxt{i})
            end
            %set color map based on user selection
            cmap = cmap_selection;
            if isempty(cmap), cmap = 'parula'; end
            colormap(cmap)
            %title(t,sprintf('Shoreline Movement using %d profiles',np));
            title(t,'Shoreline movement along Pevensey Frontage')

            % nested function ---------------------------------------------
            function setPlot(i,x,y,z,ptxt)
                ht(i) = nexttile(t);
                contourf(x,y,z'); 
                %ht(i).XTick=p;                
                if np<30
                    ht(i).XTickLabel = meta.proflist;
                else
                    % ht(i).XTickLabel = string(p');
                    % ht(i).XTickLabelRotation = 0;
                end
                ht(i).XLabel.String = 'Profile No.';
                ht(i).YLabel.String = 'Elevation (mOD)';
                hc = colorbar;
                hc.Label.String = ptxt;
            end   
        end
%%
        ...
        function dsp = modelDSproperties(~,id_model) 
            ...
            switch id_model
                ...
                case 3              %Shoreline position at a profile
                    dsp.Variables = struct(...                       
                        ...
                        'QCflag',{'model','model'});
                case {4,5,6}              %Shoreline as a vector
                    dsp.Variables = struct(...                       
            ...
            if any(id_model==[3,4])
                dsp.Dimensions = struct(...    
                    ...
                    'Label',{'Profile ID'},...
                    'Format',{''});
            elseif any(id_model==5)
                dsp.Dimensions = struct(...    
                    'Name',{'PID','zLevel'},...
                    'Description',{'Profile ID','Elevation'},...
                    'Unit',{'-','mOD'},...
                    'Label',{'Profile ID','Elevation (mOD)'},...
                    'Format',{'',''});   
            else
                dsp.Dimensions = struct(...    
                    'Name',{''},...
